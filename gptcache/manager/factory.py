...
def get_data_manager(
    cache_base: Union[CacheBase, str] = None,   #Union[CacheBase, str] 表示 cache_base 可以是 CacheBase 类型的实例，或者是字符串类型。Union 表示这两种类型中的任意一种都是有效的。
    vector_base: Union[VectorBase, str] = None,
    object_base: Union[ObjectBase, str] = None,
    max_size: int = 1000,   ##默认1000条数据
    clean_size: int = None,
    eviction: str = "LRU",  #使用LRU策略进行驱逐 最近最少使用
    ...
    :param vector_base: a VectorBase object, or the name of the vector storage, it is support 'milvus', 'faiss' and
                        'chromadb' now.
                        一个VectorBase对象，或者向量存储的名称，它支持'milvus'，'faiss'和
                         现在是“chromadb”。
    :type vector_base: :class:`VectorBase` or str
    :param object_base: a object storage, supports local path and s3.   对象存储，支持本地路径和s3。
    :type object_base: :class:`ObjectBase` or str
    :param max_size: the max size for the cache, defaults to 1000.  缓存的最大大小，默认为 1000。
    :type max_size: int
    :param clean_size: the size to clean up, defaults to `max_size * 0.2`.
    :type clean_size: int
    :param eviction: the eviction policy, it is support "LRU" and "FIFO" now, and defaults to "LRU".    驱逐策略，现在支持“LRU”和“FIFO”，默认为“LRU”。
    :type eviction:  str
    :param data_path: the path to save the map data, defaults to 'data_map.txt'.地图数据保存路径，默认为“data_map.txt”。
    :type data_path:  str
    :param get_data_container: a Callable to get the data container, defaults to None.
    :type get_data_container:  Callable
