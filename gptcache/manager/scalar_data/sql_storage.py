...
def get_models(table_prefix, db_type, table_len_config):
    ...
    class QuestionTable(DynamicBase):
        ...
        timeLeft = Column(Integer, default=60)
...
class SQLStorage(CacheStorage):
    ...
    使用 sqlalchemy 管理 SQLite、PostgreSQL、MySQL、MariaDB、SQL Server 和 Oracle。
    ...
    def get_data_by_id(self, key: int) -> Optional[CacheData]:
        with self.Session() as session:
            ...
            qs.last_access = datetime.now() #更新存在时间
            if(timeLeft>5):
                qs.timeLeft=timeLeft/2
            ans = (
